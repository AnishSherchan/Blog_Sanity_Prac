...
export default async function Home() {
  const EVENTS_QUERY = `*[_type == "blog"][0..2]`;
  const blogs = await client.fetch<SanityDocument[]>(EVENTS_QUERY);
  const builder = imageUrlBuilder(client);

  console.log(builder.image(blogs[0].poster).url());
  return (
    <>
  ...
  "
      ...
      <div className="bg-grey-50" id="blog">
        ...
        <Link href={`/blog`}>
          <span className="  block rounded-full border-2 border-blue-600 px-6 py-2 text-center font-body text-sm font-bold uppercase text-blue-600 md:text-base">
            Read More
          </span>
        </Link>
      </div>
